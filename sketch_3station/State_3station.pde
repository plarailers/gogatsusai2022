...
class State {
  ...
  State() {
    ...
    junctionList.add(new Junction(3, 0));  // servoモーターあり
    junctionList.add(new Junction(4, -1));
    junctionList.add(new Junction(5, -1));
    // Section(id, length, sourceId, targetId, sourceServoState)
    sectionList = new ArrayList<Section>();
    sectionList.add(new Section(0, (int)(STRAIGHT_UNIT * 5), 5, 0, ServoState.NoServo));
    sectionList.add(new Section(1, (int)(STRAIGHT_UNIT * 2 + CURVE_UNIT * 1), 0, 1, ServoState.NoServo));
    sectionList.add(new Section(2, (int)(STRAIGHT_UNIT * 2 + CURVE_UNIT * 2), 1, 2, ServoState.NoServo));
    sectionList.add(new Section(3, (int)(STRAIGHT_UNIT * 3 + CURVE_UNIT * 1), 2, 3, ServoState.NoServo));
    sectionList.add(new Section(4, (int)(STRAIGHT_UNIT * 5), 3, 4, ServoState.Straight));
    sectionList.add(new Section(5, (int)(STRAIGHT_UNIT * 5), 3, 4, ServoState.Curve));
    sectionList.add(new Section(6, (int)(STRAIGHT_UNIT * 3 + CURVE_UNIT * 4), 4, 5, ServoState.NoServo));
    // Sensor(id, sectionId, position)
    sensorList = new ArrayList<Sensor>();
    sensorList.add(new Sensor(0, 3, (int)(STRAIGHT_UNIT * 1.5 + CURVE_UNIT * 1)));
    sensorList.add(new Sensor(1, 6, (int)(STRAIGHT_UNIT * 0.5 + CURVE_UNIT * 2)));
    // Station(id, name)
    stationList = new ArrayList<Station>();
    stationList.add(new Station(0, "駒場"));
    stationList.add(new Station(1, "本郷"));
    stationList.add(new Station(2, "柏"));
    // station.setTrack(trackId, sectionId, stationPosition)
    Station.getById(0).setTrack(1, 0, (int)(STRAIGHT_UNIT * 3));  // 駅0の1番線はSection0
    Station.getById(1).setTrack(1, 2, (int)(STRAIGHT_UNIT * 1.5 + CURVE_UNIT * 1));  // 駅1の1番線はsection2
    Station.getById(2).setTrack(1, 4, (int)(STRAIGHT_UNIT * 3));  // 駅2の1番線はsection4
    Station.getById(2).setTrack(2, 5, (int)(STRAIGHT_UNIT * 3));  // 駅2の2番線はsection5
    ...
    trainList.add(new Train(1, Station.getById(2).trackList.get(2), (int)(STRAIGHT_UNIT * 3)));  // 駅1の2番線に配置
